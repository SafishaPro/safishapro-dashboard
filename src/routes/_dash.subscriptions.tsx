...
import { bookingsApi, servicesApi, usersApi, type ApiUser, type Service, type Subscription, type PropertyType } from "@/lib/api";
...
function SubscriptionsPage() {
  ...
  return <div className="space-y-6">
    ...
    <Dialog open={details !== null} onOpenChange={(value) => { if (!value) setDetails(null); }}><DialogContent className="max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>{details?.subscription_name || "Subscription details"}</DialogTitle></DialogHeader>{details && <div className="space-y-5"><div className="flex items-center justify-between"><p className="text-lg font-medium">{details.customer_name || "Customer unavailable"}</p><Badge variant={details.status === "active" ? "success" : "secondary"}>{details.status}</Badge></div><dl className="grid grid-cols-2 gap-4 text-sm">{[
