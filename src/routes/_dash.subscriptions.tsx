...
function SubscriptionsPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-4"><Table><TableHeader><TableRow><TableHead>Subscription</TableHead><TableHead>Billing cycle</TableHead><TableHead>Next service</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Price per cycle</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{subscriptions.map((item) => <TableRow key={item.id}><TableCell><p className="font-mono text-xs">{item.id}</p><p className="mt-1 text-xs text-muted-foreground">Customer: {item.customer_id}</p></TableCell><TableCell>{item.billing_cycle.replaceAll("_", " ")}</TableCell><TableCell>{item.next_service_date || "—"}</TableCell><TableCell><Badge variant={item.status === "active" ? "success" : "secondary"}>{item.status}</Badge></TableCell><TableCell className="text-right">{item.currency} {Number(item.price_per_cycle).toLocaleString()}</TableCell><TableCell className="text-right"><Button size="sm" variant="destructive" disabled={item.status === "cancelled"} onClick={() => void cancel(item)}>Cancel</Button></TableCell></TableRow>)}{!subscriptions.length && <TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground">No subscriptions found.</TableCell></TableRow>}</TableBody></Table></Card>
