...
function AuditPage() {
  ...
  const exportCsv = () => {
    ...
    downloadFile(`\uFEFF${rows.map((row) => row.map(csvCell).join(",")).join("\n")}`, `clean-match-monitoring-report-${fileDate()}.csv`, "text/csv;charset=utf-8");
  };
