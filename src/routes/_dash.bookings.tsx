        ...
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          {selected && (
            <div className="space-y-4">
              ...
              {(selected.special_instructions || selected.additional_notes || selected.additional_services?.length) && <div className="rounded-lg border p-3 text-sm"><h3 className="font-medium">Service notes and add-ons</h3>{selected.special_instructions && <p className="mt-2 text-muted-foreground">{selected.special_instructions}</p>}{selected.additional_notes && <p className="mt-1 text-muted-foreground">{selected.additional_notes}</p>}{selected.additional_services?.length ? <p className="mt-2 text-xs text-muted-foreground">Add-ons: {selected.additional_services.map((service) => `${service.name_snapshot ?? service.name ?? "Service"} × ${service.quantity ?? 1}: ${service.currency ?? selected.currency} ${Number(service.total_price ?? service.amount ?? 0).toLocaleString()} (included)`).join(", ")}</p> : null}</div>}
